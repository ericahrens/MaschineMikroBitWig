...
class GeneralModeHandler {
	...
	constructor() {
		...
		this.groupButton.setCallback((value, button) => {
			if(value > 0) {
				this.groupButton.updateColor(5,127,127)
			} else {
				this.groupButton.updateColor(5,127,50)
			}
		})
	}
}
		...
		switch(type) {
			...
			case 0x9: // MIDI Note
				if(data1 > 11 && data1 <29) {
					let index = data1-12
					this.buttonMatrix.setBrightness(index, data2 > 0 ? 127 : 0)
					return false
				}
				break;
		} 
...
class ButtonMatrix {
	...
	constructor(parent: Controllers, basevalue: number) {
		...
		this.buttons = []
		for(let i=0;i<16;i++) {
			this.buttons.push(new ColorButton(this, i, basevalue+i))
		}
	}
...
class BasicColorButton extends Button {
	...
	updateColor( hue: number, sat: number, bright: number) : void {
		if(hue != this.hue) {
			this.hue = hue
			queueColor(3, this.midivalue, this.hue)
		}
		if(sat != this.sat) {
			this.sat = sat
			queueColor(4, this.midivalue, this.sat)
		}
		if(bright != this.bright) {
			this.bright = bright
			queueColor(5, this.midivalue, this.bright)
		}
	}
	...
	update(force : boolean = false) : void {
		queueColor(0, this.midivalue, this.hue)
		queueColor(1, this.midivalue, this.sat)
		queueColor(2, this.midivalue, this.bright)
	}
}
